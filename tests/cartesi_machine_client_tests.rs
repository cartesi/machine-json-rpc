...
use cartesi_machine_json_rpc::interfaces::{self, index};
...
mod local_server {
    ...
    async fn context_with_machine_future() -> Context {
        ...
        default_config.uarch = UarchConfig {
            processor: Some(index::UarchProcessorConfig {
                x: Some(vec![0; 32]),
                pc: Some(0x70000000),
                cycle: Some(0),
            }),
            ram: Some(index::UarchRAMConfig {
                length: Some(77128),
    ...
    async fn test_replace_memory_range(
        ...
        context
            .get_server()
            .replace_memory_range(index::MemoryRangeConfig::from(&memory_range_config))
            .await?;
        let ret = context
            .get_server()
